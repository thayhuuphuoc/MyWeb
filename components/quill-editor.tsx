...
const QuillEditor = React.forwardRef<HTMLTextAreaElement, TextareaProps>((props, ref) => {
	...
	useEffect(() => {
		...
		// Find selected cell in table
		const findSelectedCell = (table: HTMLElement): HTMLElement | null => {
			// Try direct query first
			let selectedCell = table.querySelector('td.ql-cell-selected, th.ql-cell-selected, td.ql-cell-focused, th.ql-cell-focused') as HTMLElement | null
			
			if (!selectedCell) {
				// Fallback: iterate through all cells
				const allCells = Array.from(table.querySelectorAll('td, th'))
				for (const cell of allCells) {
					const cellEl = cell as HTMLElement
					if (cellEl.classList.contains('ql-cell-selected') || cellEl.classList.contains('ql-cell-focused')) {
						selectedCell = cellEl
						break
					}
				}
		...
		// Read border values from form container
		const readBorderFromForm = (formContainer: HTMLElement | null | undefined): { borderStyle: string, borderColor: string, borderWidth: string } => {
			const result = { borderStyle: '', borderColor: '', borderWidth: '' }
			
			if (!formContainer) return result
			
			const searchRoot = formContainer
			
			// Read border style from dropdown
			const borderStyleDropdown = searchRoot.querySelector('.ql-table-dropdown-properties[data-property="border-style"], [data-property="border-style"] .ql-table-dropdown-properties') as HTMLElement
			if (borderStyleDropdown) {
				...
				if (selectedItem) {
					result.borderStyle = selectedItem.getAttribute('data-value') || selectedItem.textContent?.trim() || ''
				} else {
					const dropdownText = borderStyleDropdown.querySelector('.ql-table-dropdown-text') as HTMLElement
					if (dropdownText) {
						result.borderStyle = dropdownText.textContent?.trim() || ''
					}
				}
			}
			
			// Read border color
			const borderColorInput = searchRoot.querySelector('[data-property="border-color"] .property-input, .ql-table-color-container[data-property="border-color"] .property-input') as HTMLInputElement
			if (borderColorInput) {
				result.borderColor = borderColorInput.value || borderColorInput.getAttribute('value') || ''
				if (!result.borderColor) {
					const colorButton = searchRoot.querySelector('[data-property="border-color"] .color-button') as HTMLElement
					if (colorButton) {
						result.borderColor = colorButton.getAttribute('data-color') || colorButton.style.backgroundColor || ''
					}
				}
			}
			
			// Read border width
			const borderWidthInput = searchRoot.querySelector('[data-property="border-width"] .property-input, .label-field-view[data-property="border-width"] .property-input') as HTMLInputElement
			if (borderWidthInput) {
				result.borderWidth = borderWidthInput.value || borderWidthInput.getAttribute('value') || ''
			}
			
			return result
		}

		// Apply border with retry logic
		const applyBorderWithRetry = (table: HTMLElement, borderStyle?: string, borderColor?: string, borderWidth?: string, selectedCell?: HTMLElement | null, maxAttempts: number = 5) => {
			const tryApply = (attempt: number) => {
				setTimeout(() => {
					if (!selectedCell) {
						selectedCell = findSelectedCell(table)
					}
					
					if (borderStyle || borderColor || borderWidth) {
						applyBorderToCellsWithValues(table, borderStyle, borderColor, borderWidth, selectedCell || null)
					} else {
						const tableStyle = table.getAttribute('style') || ''
						applyBorderToCells(table)
						// Retry if style doesn't seem updated yet
						if (attempt < maxAttempts && (!tableStyle.includes('border-style') || (tableStyle.includes('border-style: solid') && tableStyle.includes('#000000')))) {
							tryApply(attempt + 1)
						}
					}
				}, attempt * 200)
		...
		const restoreBorderToTableForForm = (table: HTMLElement) => {
			...
			// CRITICAL: Always restore border to style attribute, even if it exists
			// This ensures form can always read the correct values
			const currentStyle = table.getAttribute('style') || ''
			
			// Remove existing border properties first
			...
			const newStyle = cleanedStyle ? `${cleanedStyle}; ${borderStyleStr}` : borderStyleStr
		...
		const applyBorderToCellsWithValues = (table: HTMLElement, borderStyle?: string, borderColor?: string, borderWidth?: string, selectedCell?: HTMLElement | null) => {
			...
			if (!selectedCell) {
				...
				warn('No selected cell found, applying border to all cells')
			...
			if (borderStyle && borderStyle !== 'none' && borderColor && borderWidth) {
				...
				setTimeout(() => {
					...
					if (styleEl) {
						log(`Dynamic style verified for ${cellId ? 'cell' : 'table'} ${tableId}${cellId ? `-${cellId}` : ''} (from applyBorderToCellsWithValues)`)
					} else {
						error(`Dynamic style NOT found for ${cellId ? 'cell' : 'table'} ${tableId}${cellId ? `-${cellId}` : ''} (from applyBorderToCellsWithValues)`)
					}
				}, 10)
			}
			...
			cells.forEach((cell, index) => {
				...
				if (isFirstCell && borderStyle && borderStyle !== 'none' && borderColor && borderWidth) {
					...
					setTimeout(() => {
						...
						if (computed.borderTopWidth === '0px' || computed.borderLeftWidth === '0px' || 
							...
							warn('First cell border still missing (applyBorderToCellsWithValues), forcing again')
							forceFirstCellBorder()
						}
					}, 100)
				}
				...
				if (index === 0) {
					...
					const computed = window.getComputedStyle(cellEl)
					log('applyBorderToCellsWithValues - Cell 0 debug:', {
			...
			log('Applied border to cells with values:', { borderStyle, borderColor, borderWidth, cellsCount: cells.length, appliedCount })
		}
		...
		const createDynamicStyleForCell = (cell: HTMLElement, borderStyle: string, borderColor: string, borderWidth: string) => {
			...
			log(`Created dynamic style for cell ${cellId} in table ${tableId}:`, { borderStyle, borderColor, borderWidth })
		}
		...
		const createDynamicStyleForTable = (table: HTMLElement, borderStyle: string, borderColor: string, borderWidth: string) => {
			...
			if (borderStyle && borderColor && borderWidth) {
				...
				log(`Created dynamic style for table ${tableId}:`, { borderStyle, borderColor, borderWidth })
				// Log the actual CSS rule created
				log(`Dynamic style CSS:`, styleEl.textContent)
			}
		}
			...
			cells.forEach((cell, index) => {
				...
				if (isFirstCell && borderStyle && borderStyle !== 'none' && borderColor && borderWidth) {
					...
					setTimeout(() => {
						...
						if (computed.borderTopWidth === '0px' || computed.borderLeftWidth === '0px' || 
							computed.borderTopWidth === '' || computed.borderLeftWidth === '') {
							warn('First cell border still missing, forcing again')
							forceFirstCellBorder()
						}
					}, 100)
				}
				...
				if (index === 0) {
					...
					if (isDev) {
						log('=== Cell 0 Debug Info ===')
						log('Style Attribute:', styleAttr)
						log('Inline Border Style:', cellEl.style.getPropertyValue('border-style'))
						log('Inline Border Color:', cellEl.style.getPropertyValue('border-color'))
						log('Inline Border Width:', cellEl.style.getPropertyValue('border-width'))
						log('Computed Border Style:', computed.borderStyle)
						log('Computed Border Color:', computed.borderColor)
						log('Computed Border Width:', computed.borderWidth)
						log('Computed Border Top:', computed.borderTop)
						log('Computed Border Right:', computed.borderRight)
						log('Computed Border Bottom:', computed.borderBottom)
						log('Computed Border Left:', computed.borderLeft)
						log('Expected Style:', borderStyle)
						log('Expected Color:', borderColor)
						log('Expected Width:', borderWidth)
						log('Table ID:', tableId)
						log('Table Has ID:', tableHasId)
						log('Dynamic Style Exists:', !!dynamicStyleEl)
						log('Cell Matches Selector:', cellMatchesSelector)
						log('Parent Chain:', parentChain.join(' > '))
						log('Table Classes:', table?.className)
						log('Table Attributes:', table ? Array.from(table.attributes).map(attr => `${attr.name}="${attr.value}"`).join(', ') : 'no table')
						log('========================')
					}
				}
			})
		...
		if (tableBetter && tableBetter.tableMenus) {
			...
			setTimeout(() => {
				...
				// CRITICAL: Patch getter to restore border before form is accessed
				if (originalGetTablePropertiesForm) {
					Object.defineProperty(Object.getPrototypeOf(tableMenus), 'tablePropertiesForm', {
						get: function() {
							// Restore border to table before form reads values
							if (this.table) {
								restoreBorderToTableForForm(this.table as HTMLElement)
							}
							return originalGetTablePropertiesForm.call(this)
						},
						configurable: true,
						enumerable: true
					})
				...
				if (tableMenus.tablePropertiesForm) {
					...
					if (form && form.saveTableAction) {
						...
						form.saveTableAction = function() {
							...
							const { borderStyle, borderColor, borderWidth } = readBorderFromForm(form.container)
							
							log('Save table action: border properties from form:', { borderStyle, borderColor, borderWidth })
							...
							if (table) {
								applyBorderWithRetry(table as HTMLElement, borderStyle || undefined, borderColor || undefined, borderWidth || undefined)
							}
						}
					}
				}
			...
			if (originalCreateTablePropertiesForm) {
				tableBetter.tableMenus.createTablePropertiesForm = function(type: string) {
					...
					// Do this BEFORE calling originalCreateTablePropertiesForm so form can read values
					if (this.table && type === 'table') {
						const table = this.table as HTMLElement
						restoreBorderToTableForForm(table)
						// Also mark that form is opening to prevent MutationObserver from removing border
						table.setAttribute('data-form-opening', 'true')
					}
					const result = originalCreateTablePropertiesForm.call(this, type)
					// After form is created, set a flag to keep border visible
					if (this.table && type === 'table') {
						const table = this.table as HTMLElement
						table.setAttribute('data-form-open', 'true')
						table.removeAttribute('data-form-opening')
					}
					...
					setTimeout(() => {
						if (this.tablePropertiesForm && this.tablePropertiesForm.saveTableAction) {
							...
							this.tablePropertiesForm.saveTableAction = function() {
								...
								const { borderStyle, borderColor, borderWidth } = readBorderFromForm(formContainer)
								
								log('Save table action (from createForm): border properties from form:', { borderStyle, borderColor, borderWidth })
			...
			setTimeout(() => {
				...
				if (tableMenus.tablePropertiesForm) {
					...
					if (form && form.saveTableAction) {
						const originalSaveTableAction = form.saveTableAction.bind(form)
						form.saveTableAction = function() {
							log('saveTableAction called (patch 1)')
							
							// Get border properties from form inputs BEFORE saving
							const { borderStyle, borderColor, borderWidth } = readBorderFromForm(form.container)
							
							log('Save table action (patch 1): border properties from form:', { borderStyle, borderColor, borderWidth })
							...
							const { table } = this.tableMenus
							if (table) {
								applyBorderWithRetry(table as HTMLElement, borderStyle || undefined, borderColor || undefined, borderWidth || undefined, undefined, 3)
							}
						}
					}
				}
				...
				tableBetter.tableMenus.createTablePropertiesForm = function(type: string) {
					...
					setTimeout(() => {
						if (this.tablePropertiesForm && this.tablePropertiesForm.saveTableAction) {
							const originalSave = this.tablePropertiesForm.saveTableAction.bind(this.tablePropertiesForm)
							this.tablePropertiesForm.saveTableAction = function() {
								log('saveTableAction called (patch 2)')
								// Get border properties from form inputs BEFORE saving
								const formContainer = this.container || this.element
								const { borderStyle, borderColor, borderWidth } = readBorderFromForm(formContainer)
								
								log('Save table action (patch 2): border properties from form:', { borderStyle, borderColor, borderWidth })
			...
			mutations.forEach((mutation) => {
				if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
					...
					if (target.tagName === 'TABLE') {
						...
						if (isFromForm) {
							log('MutationObserver: Border applied from form - applying to cells but keeping border on table for edit')
						...
						if (tableStyle.includes('border-style') || tableStyle.includes('border-color') || tableStyle.includes('border-width')) {
							log('MutationObserver: table style changed', tableStyle)
		...
		// Observe all tables for style changes
		const tables = quill.root.querySelectorAll('table')
		tables.forEach((table) => {
