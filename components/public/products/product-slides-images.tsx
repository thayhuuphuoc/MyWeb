...
export default function ProductSlideImages({data}: {
	...
	return (
		<div className={''}>
			<picture
				className={'cursor-zoom-in rounded-md block w-full overflow-hidden aspect-square relative border border-gray-200 mb-2 bg-white/50'}
				onClick={() => {
				...
				>
					{parseProductImages(data.images).map((image, index)=> (
						...
						>
							<picture
								data-src={image.url}
								className={cn('slick__slide pointer-events-none rounded-md block mb-0 overflow-hidden w-16 aspect-square relative transition-colors bg-white/50', {
