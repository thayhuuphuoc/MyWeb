...
export default function ProductCard({product, size}: {
	...
	return (
		<div className={cn('product-item relative bg-vweb_bg flex-col justify-start items-start gap-1 p-3 rounded-lg inline-flex border border-indigo-200 border-opacity-50 gradient-border', {
			...
			<div className={'self-stretch flex-col justify-start items-start gap-5 flex mt-3 mb-1'}>
				<h3 title={product.title} className={cn("font-medium m-0 leading-snug line-clamp-2", {
					"text-sm md:text-base": size === 'sm',
					"xl:text-lg": size !== 'sm'
				})}>
					<Link href={`/san-pham/${product.slug}`} className={'transition-all duration-150 hover:text-blue-500'}>
						{product.title}
					</Link>
				</h3>
			</div>
